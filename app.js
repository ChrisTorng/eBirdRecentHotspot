...
function render(groups) {
  ...
  for (const name of ['count-col', 'latest-col', 'average-col', 'species-col', 'date-col', 'observer-col']) columns.append(element('col', null, name));
  table.append(columns);
  const head = element('thead'), header = element('tr');
  for (const [title, hint, className] of [
    ['紀錄 / 人', '全部清單 / 不同鳥友名稱', 'numeric'],
    ['最近日期', '此地點最新觀察日', 'date-cell'],
    ['平均鳥種 / 紀錄', '最近一天的平均 / 筆數', 'numeric'],
    ['鳥種', '單筆', 'numeric'], ['日期', '觀察時間', 'date-cell'], ['鳥友名', '地點展開後顯示', '']
  ]) {
  ...
  groups.forEach((group, index) => {
    ...
    const button = element('button', '▸', 'toggle-group'); button.type = 'button';
    button.setAttribute('aria-expanded', 'false');
    button.setAttribute('aria-label', `展開 ${group.name} 的 ${group.count} 筆紀錄`);
    count.append(button, pair(group.count, group.observers));
    ...
    const name = group.hotspot ? link(displayName, `https://ebird.org/hotspot/${encodeURIComponent(group.id)}/bird-list?yr=curM`) : element('span', displayName);
    name.title = group.name;
    name.className = 'location-name';
    location.append(name);
    tr.append(count, latest, average, location); body.append(tr);
    const detailRows = group.rows.map((row, rowIndex) => {
      ...
      speciesCell.append(row.subId ? link(species, `https://ebird.org/checklist/${encodeURIComponent(row.subId)}`) : element('span', species));
      const date = element('td', row.date ? `${shortDate(row.date)}${row.date.includes('T') ? ' ' + row.date.slice(11, 16) : ''}` : '日期不明', 'date-cell'); date.title = row.date;
      detail.append(blank, speciesCell, date, element('td', row.userDisplayName || '未提供', 'observer-cell'));
...
async function load() {
  ...
  try {
    ...
    $('summary').textContent = `${groups.length} 個地點 · ${groups.reduce((sum, g) => sum + g.count, 0)} 筆紀錄`;
