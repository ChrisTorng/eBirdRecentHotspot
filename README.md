...
- `main` 放前端、scripts、tests、examples；`data` branch 放 `data/index.json`、`locations.json` 、`snapshots/YYYY-MM-DD.json` 與 `summaries/YYYY-MM-DD.json`。
...
同地點、完整時間、鳥種數相同的清單合併；缺時間或鳥種數不合併。紀錄數仍按原始清單數（去除重複 ID，每個人的清單仍各算一筆），排序改以固定三日近期熱度；鳥友數按不同顯示名稱，平均鳥種為最近一天合併紀錄的平均，不是鳥種聯集。

地點名稱與箭頭共同展開，鳥點及 Google Maps 座標連結在右側，手機上下排列。個人鳥點僅顯示文字，不提供鳥點或鳥圖連結。日期連至第一人清單；多人合併列各鳥友各自連結，單人列鳥友名純文字。外部連結另開並有 ↗；個人地點不產生公開 hotspot 連結。
...
6. Push main、workflow_dispatch、台灣每日 10:00（UTC `0 2 * * *`）執行，排程可能延遲。data push 不觸發。本專案不放 CNAME，沿用使用者 Pages 自訂網域。
...
熱門指標與曲線已實作，完整規則見 [熱度計算與資料流程](docs/hotness-proposal.md)。

## 熱度與每日摘要

主列顯示鳥點名稱、近期熱度、14 天曲線、鳥點／地圖連結。展開後查看原有統計、每日數值與資料品質，以及所選天數的清單。

每日同行組以不同顯示名稱計算 `1 + 0.5 × (人數 − 1)`，平均分給各人；同人同地同日只取最高一次，再加總為當日熱度。缺名稱不虛構人數，標示不完整。

排序固定取截至最後日期的三日，權重 50%／30%／20%，無額外連續加成或退燒扣分。最後日期為今天時包含今天截至擷取時間的累計，標為暫計；歷史日期包含該日。展示天數只改變清單範圍與候選鳥點，不改變共同鳥點的分數。完整分數由高到低；資料不足排在後方，同分以地點 ID 固定排序。

曲線固定 14 天，使用按日輕量摘要，無需載入額外天數的完整清單。各曲線使用目前篩選結果內所有鳥點、所有曲線日期的共同最大值，方便比較高度。今天或不完整點空心／虛線；缺日斷線。未日結但未截斷的 feed（open）以截至擷取時的累計參與評分，沒有紀錄暫計零；縣市達 200 上限、舊快照、缺鳥友名稱仍視為資料不足，不能把截斷缺漏當零。

`collect` 完成後重建摘要；`build` 也會重建，Actions 將摘要隨 data branch 提交。`npm run examples` 同樣產生摘要。摘要保留人數、清單數、品質及算法版本，不保存鳥友名稱；原始精簡清單仍在，可供日後調整算法重新計算。單獨重建可執行 `node scripts/summaries.mjs .local-data`。

近期熱度標題提供共用算法提示，各筆分數 tooltip 顯示實際三日數值、加權算式及擷取時間。公開鳥點另提供「鳥圖」連至 `https://ebird.org/hotspots?hs=地點ID`，外部連結均另開。

手動更新：GitHub → Actions → Collect and publish → Run workflow → 選擇 main → Run workflow。成功完成 collect 與 deploy 後，網站會使用新資料。
