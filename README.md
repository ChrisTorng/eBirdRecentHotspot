...
排序固定取截至最後日期的三個已結束日，權重 50%／30%／20%，無額外連續加成或退燒扣分。最後日期為今天時從昨天算起；歷史日期包含該日。展示天數只改變清單範圍與候選鳥點，不改變共同鳥點的分數。完整分數由高到低；資料不足排在後方，同分以地點 ID 固定排序。

曲線固定 14 天，使用按日輕量摘要，無需載入額外天數的完整清單。各曲線縱軸由零起、標示各自最高值。今天或不完整點空心／虛線；缺日斷線。縣市達 200 上限、舊快照、未在當日結束後更新、缺鳥友名稱，均不能視為完整評分來源；缺席也不能在不完整縣市當成零。
